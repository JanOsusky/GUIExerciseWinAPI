...
void mypaintObject(HWND hWnd, HDC hDC, PAINTSTRUCT ps, int posX, int posY, POINT cursorPosition);
    ...
    LPARAM lParam) // second message parameter
{
    ...
    case WM_PAINT:
        ...
        mypaintObject(hWnd, hDC, ps, posX, posY, cursorPosition);
...
void paintObject(HWND hWnd, HDC hDC, PAINTSTRUCT ps, int posX, int posY, POINT cursorPosition)
{
    ...
    SetDCBrushColor(hDC, RGB(135, 206, 235));
    HPEN pen = CreatePen(PS_SOLID, 5, RGB(135, 206, 235));
    SelectObject(hDC, pen);

    Rectangle(hDC, 0, 0, 1000, 550); // sky

    pen = CreatePen(PS_SOLID, 5, RGB(76, 200, 0));
    SelectObject(hDC, pen);

    SelectObject(hDC, GetStockObject(DC_BRUSH));
    SetDCBrushColor(hDC, RGB(76, 200, 0));

    Rectangle(hDC, 0, 550, 1000, 700); // ground
    ...
    Rectangle(hDC, 420, 500, 520, 550); // ramp
...
{
    SelectObject(hDC, GetStockObject(DC_BRUSH));
    SetDCBrushColor(hDC, RGB(175, 175, 175));
    HPEN pen = CreatePen(PS_SOLID, 5, RGB(175, 175, 175));
    SelectObject(hDC, pen);

    Rectangle(hDC, posX - 30, posY - 100, posX, posY); // main part

    POINT verticesTip[] = {{posX - 30, posY - 100}, {posX - 15, posY - 120}, {posX, posY - 100}}; // tip
    Polygon(hDC, verticesTip, 3);

    POINT verticesLeft[] = {{posX - 50, posY}, {posX - 40, posY - 30}, {posX - 30, posY}}; // left engine
    Polygon(hDC, verticesLeft, 3);

    POINT verticesRight[] = {{posX + 20, posY}, {posX + 10, posY - 30}, {posX, posY}}; // right engine
    Polygon(hDC, verticesRight, 3);

    SelectObject(hDC, GetStockObject(DC_BRUSH));
    SetDCBrushColor(hDC, color1);
    pen = CreatePen(PS_SOLID, 5, color1);
    SelectObject(hDC, pen);

    POINT verticesLFire[] = {{posX - 45, posY}, {posX - 40, posY + 10}, {posX - 35, posY}}; // fire left
    Polygon(hDC, verticesLFire, 3);

    POINT verticesSFire[] = {{posX - 20, posY}, {posX - 15, posY + 10}, {posX - 10, posY}}; // fire s
    Polygon(hDC, verticesSFire, 3);

    POINT verticesRFire[] = {{posX + 15, posY}, {posX + 10, posY + 10}, {posX + 5, posY}}; // fire right
    Polygon(hDC, verticesRFire, 3);
    return;
}
